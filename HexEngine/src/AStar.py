...
class AStar:
    ...
    def find_path(self,map, start_pos, end_pos):
        """
        ...
        """
        #check if end pos is in range
        if HexMath.checkInGrid(end_pos[0]+1,end_pos[1] , map.x,map.y) == False:
            print "Out of bounds"
            return {}
        start_pos = (start_pos[0]+1,start_pos[1])#+1 for dummy col
        
        
        open = SortedDictionary(sort_function = self.sort_function)
        closed = SortedDictionary()
        
        #set node proberty to end
        
        map.getTilebyHex(end_pos).set_property(Tile.Property.END)

        # Shortened names
        get_adjacent_positions = HexMath.getNeighboursB
        ...
        current_node = map.getTilebyHex(start_pos)
        ...
        while not current_node.is_end():
            ...
            if len(open) == 0:
                ...
                adjacent_positions = get_adjacent_positions(current_pos,map)
                ...
                for adjacent_pos in adjacent_positions:
                    ...
                    adjacent_node = map.getTilebyHex(adjacent_pos)
                    ...
                    if (adjacent_node.get_property() <> p.BARRIER and
                        ...
                        if open.has_key(adjacent_pos):
                            ...
                            map.getTilebyHex(adjacent_pos).set_parent_and_score(current_pos,
                                                               current_node.g,
                                                               end_pos)
        ...
        path_pos = map.getTilebyHex(end_pos).parent
        path = []
        while path_pos <> None:
            path.append(path_pos)
            path_pos = map.getTilebyHex(path_pos).parent
